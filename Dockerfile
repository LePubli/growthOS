...
# --no-frozen-lockfile avoids strict lockfile version checks in CI environments
...
# Build the Vite SPA
# PORT is required by vite.config.ts at load time (only used for dev server config)
RUN PORT=3000 \
    BASE_PATH=${BASE_PATH} \
