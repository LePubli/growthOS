...
import { User, Users, Key, CreditCard, Link as LinkIcon, ChevronRight, ArrowLeft, Shield, Bell, Globe, Palette } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen p-6" style={{background:'var(--body-bg)'}}>
      <div className="flex items-center gap-3 mb-6">
        <button onClick={()=>router.back()} className="p-2 rounded-xl hover:opacity-80 transition-all" style={{background:'var(--card-bg)',border:'1px solid var(--card-border)',color:'var(--text-secondary)'}}>
      ...
      <div className="max-w-2xl space-y-3">
        {SECTIONS.map(s=>(
          <Link key={s.href} href={s.href} className="flex items-center gap-4 rounded-2xl border p-5 hover:shadow-md transition-all block"
            ...
            <div className="flex-1">
              ...
              <div className="text-sm" style={{color:'var(--text-muted)'}}>{(s as any).desc||s.title}</div>
            </div>
