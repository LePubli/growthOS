...
import { Star, Download, CheckCircle, Search, Zap, Users, Mail, BarChart2, Bot, Globe, Puzzle, Shield, X, ChevronRight, TrendingUp, Clock, Package, Sparkles, Check } from 'lucide-react';
import apiClient from '@/lib/api-client';

/* ─── Pack definitions (mirrored from backend) ─────────────────────────── */
const PACKS = {
  "sales-intelligence": {
    ...
    color: "#4F46E5",
    bg: "#EEF2FF",
    features: ["Mémoire contextuelle IA", "Intelligence réunions", "Fiches compte 360°", "Coach deal en temps réel"],
  },
  "automation-pro": {
    id: "automation-pro",
    ...
    color: "#0891B2",
    bg: "#ECFEFF",
    features: ["SDR IA automatisé", "Détection de signaux", "Séquences intelligentes", "Scoring automatique"],
  },
  "executive-suite": {
    id: "executive-suite",
    name: "Pack Direction",
    ...
    color: "#D97706",
    bg: "#FEF3C7",
    features: ["Dashboard revenus temps réel", "Centre de commande exécutif", "Forecasting IA", "KPIs avancés"],
  },
};

/* ─── Individual marketplace plugins ────────────────────────────────────── */
const CATEGORIES = [
...
const PLUGINS = [
  { id:'crm-map', name:'CRM Map & Tournée', description:'Visualisez vos prospects sur une carte interactive. Planifiez vos tournées commerciales avec optimisation de trajets.', longDesc:'Optimisez vos déplacements commerciaux grâce à une carte interactive avec clustering, zones de chalandise, calcul de tournée optimale (algorithme TSP) et intégration GPS.', version:'1.0.0', category:'crm', rating:4.9, reviews:142, installs:3800, price:'Gratuit', icon:'🗺️', featured:true, tags:['carte','tournée','géolocalisation'], author:'GrowthOS Labs', updatedAt:'Il y a 3j', installed:true },
  { id:'linkedin-enricher', name:'LinkedIn Enricher', description:'Enrichissement automatique des profils prospects avec données LinkedIn — poste, entreprise, ancienneté.', longDesc:'Connectez votre compte LinkedIn Sales Navigator pour enrichir automatiquement vos contacts. Mise à jour hebdomadaire, score de confiance par champ.', version:'2.1.0', category:'enrichment', rating:4.8, reviews:98, installs:1240, price:'29€/mois', icon:'💼', featured:true, tags:['linkedin','enrichissement'], author:'DataFlow SAS', updatedAt:'Il y a 1sem', installed:false },
  { id:'email-verifier', name:'Email Verifier Pro', description:'Vérifiez la délivrabilité de vos emails en temps réel avec Hunter.io, Bouncer et NeverBounce.', longDesc:'Réduisez votre taux de rebond en vérifiant chaque email avant envoi. Vérification SMTP, MX record, catch-all, disposable address detection.', version:'3.0.2', category:'enrichment', rating:4.7, reviews:67, installs:890, price:'19€/mois', icon:'✉️', featured:false, tags:['email','délivrabilité'], author:'MailGuard', updatedAt:'Il y a 2sem', installed:true },
  { id:'revenue-analytics', name:'Revenue Analytics', description:'Tableaux de bord avancés sur votre performance commerciale : ARR, MRR, churn, LTV et forecasting IA.', longDesc:'Connectez votre CRM pour obtenir des métriques SaaS avancées. Segmentation par industrie, taille, géographie.', version:'1.5.0', category:'analytics', rating:4.6, reviews:54, installs:567, price:'49€/mois', icon:'📈', featured:true, tags:['analytics','revenu','SaaS'], author:'Metric Labs', updatedAt:'Il y a 5j', installed:false },
  { id:'zapier-connect', name:'Zapier Connect', description:'Plus de 5 000 intégrations via Zapier. Synchronisez GrowthOS avec Slack, HubSpot, Salesforce et plus.', longDesc:'Déclenchez des zaps depuis GrowthOS : nouveau prospect, deal gagné, signal détecté. Configuration sans code en moins de 5 minutes.', version:'2.0.1', category:'integrations', rating:4.5, reviews:211, installs:4200, price:'Gratuit', icon:'⚡', featured:false, tags:['zapier','automation','intégration'], author:'Zapier Inc.', updatedAt:'Il y a 1j', installed:false },
  { id:'ai-copywriter', name:'AI Copywriter', description:'Génération automatique d\'emails de prospection personnalisés avec GPT-4. Rédaction contextuelle.', longDesc:'Créez des emails ultra-personnalisés en 1 clic. Analyse du profil LinkedIn, actualités de l\'entreprise, signal détecté.', version:'1.2.0', category:'automation', rating:4.4, reviews:39, installs:320, price:'39€/mois', icon:'🤖', featured:false, tags:['IA','copywriting','email'], author:'GrowthOS Labs', updatedAt:'Il y a 4j', installed:false },
  { id:'gdpr-shield', name:'GDPR Shield', description:'Conformité RGPD automatique : gestion des consentements, purge programmée, audit trail complet.', longDesc:'Assurez votre conformité RGPD sans effort. Gestion des droits à l\'effacement, portabilité, consentement.', version:'1.1.0', category:'security', rating:4.3, reviews:28, installs:210, price:'29€/mois', icon:'🔒', featured:false, tags:['RGPD','conformité','sécurité'], author:'LegalTech SAS', updatedAt:'Il y a 2sem', installed:false },
  { id:'slack-alerts', name:'Slack Alerts', description:'Notifications Slack en temps réel sur vos prospects, deals et signaux. Configurez des alertes personnalisées.', longDesc:'Restez informé sans quitter Slack. Configurez des webhooks pour chaque événement GrowthOS.', version:'1.0.3', category:'integrations', rating:4.8, reviews:76, installs:980, price:'Gratuit', icon:'💬', featured:false, tags:['slack','notifications'], author:'GrowthOS Labs', updatedAt:'Il y a 6j', installed:true },
  { id:'forecast-ai', name:'Forecast AI', description:'Prédiction du chiffre d\'affaires pour les 3 prochains mois grâce au machine learning sur l\'historique.', longDesc:'Modèle ML entraîné sur votre historique pour prédire le CA mensuel avec intervalle de confiance.', version:'0.9.0', category:'analytics', rating:4.2, reviews:18, installs:145, price:'59€/mois', icon:'🔮', featured:false, tags:['IA','forecast','analytics'], author:'Predictive Labs', updatedAt:'Il y a 3sem', installed:false },
];

type SortKey = 'rating' | 'installs' | 'name';

/* ─── Pack card component ───────────────────────────────────────────────── */
function PackCard({ pack, onInstall, isInstalling, isInstalled }: {
  ...
  return (
    <div style={{
      ...
      border: `2px solid ${isInstalled ? '#059669' : pack.color}22`,
      borderRadius: 18,
      padding: '20px 22px',
      display: 'flex',
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
        <div style={{
          ...
          background: pack.bg, display: 'flex', alignItems: 'center',
        ...
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <div style={{ fontSize: 16, fontWeight: 800, color: pack.color }}>{pack.price}</div>
          <div style={{ fontSize: 10, color: '#059669', fontWeight: 600, marginTop: 2 }}>{pack.savings}</div>
        </div>
      </div>
        ...
        {pack.features.map((f) => (
          <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: 'var(--text-secondary)' }}>
            <div style={{
              ...
              background: `${pack.color}18`, display: 'flex',
              alignItems: 'center', justifyContent: 'center', flexShrink: 0,
            }}>
              <Check size={9} color={pack.color} />
            </div>
            {f}
          </div>
        ))}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: 'var(--text-muted)' }}>
        <Package size={11} />
        {pack.plugins.length} plugins inclus
      </div>

      {/* CTA */}
      <button
        onClick={() => !isInstalled && onInstall(pack.id)}
        disabled={isInstalling || isInstalled}
        style={{
          width: '100%', padding: '10px', borderRadius: 11, border: 'none',
          background: isInstalled ? '#ECFDF5' : isInstalling ? '#E5E7EB' : pack.color,
          color: isInstalled ? '#059669' : isInstalling ? 'var(--text-muted)' : '#fff',
          fontSize: 13, fontWeight: 700, cursor: isInstalled || isInstalling ? 'default' : 'pointer',
          display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7,
          transition: 'opacity .15s',
        }}
      >
        {isInstalled ? (
          <><CheckCircle size={14} />Pack installé</>
        ) : isInstalling ? (
          <><span style={{ animation: 'spin 1s linear infinite', display: 'inline-block' }}>⟳</span> Installation…</>
        ) : (
          <><Sparkles size={14} />Installer le pack</>
        )}
      </button>
    </div>
  );
...
function InstallModal({ plugin, onClose, onInstalled }: { plugin: any; onClose: () => void; onInstalled: (id: string) => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
      onClick={step === 'install' ? undefined : onClose}>
      <div style={{ background: 'var(--card-bg)', borderRadius: 20, width: '100%', maxWidth: 520, padding: 26, boxShadow: '0 20px 60px rgba(0,0,0,.2)', maxHeight: '90vh', overflowY: 'auto' }}
...
export default function MarketplacePage() {
  ...
  /* Fetch live marketplace data (pack statuses from API) */
  const { data: marketplaceData } = useQuery({
    queryKey: ['marketplace'],
    queryFn: () => apiClient.get('/plugins/marketplace') as Promise<{
      packs: Array<{ id: string; allActive: boolean }>;
    }>,
    staleTime: 30_000,
  });

  /* Sync pack statuses from API */
  const liveInstalledPacks = new Set<string>([
    ...installedPacks,
    ...(marketplaceData?.packs?.filter(p => p.allActive).map(p => p.id) ?? []),
  ]);

  /* Pack install mutation */
  const installPackMutation = useMutation({
    mutationFn: (packId: string) =>
      apiClient.post('/plugins/packs/install', { packId }) as Promise<{
        packName: string;
    ...
    onSuccess: (data, packId) => {
      setInstalledPacks(s => new Set([...s, packId]));
      queryClient.invalidateQueries({ queryKey: ['marketplace'] });
      queryClient.invalidateQueries({ queryKey: ['plugins'] });
      const total = data.installedCount + data.alreadyActiveCount;
      toast.success(`${data.packName} installé — ${total} plugin${total > 1 ? 's' : ''} actif${total > 1 ? 's' : ''}`);
    },
    onError: (err: any) => {
      toast.error(`Échec de l'installation : ${err?.message ?? 'Erreur inconnue'}`);
    },
  });
  ...
  return (
    <div style={{ minHeight: '100vh', padding: 24, background: 'var(--body-bg)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: 'var(--text-muted)', margin: '2px 0 0' }}>
            {PLUGINS.length} plugins · {Object.keys(PACKS).length} packs · {installedCount} installé{installedCount > 1 ? 's' : ''}
          </p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(130px,1fr))', gap: 10, marginBottom: 20 }}>
        {[
          { l: 'Plugins installés', v: installedCount, color: '#059669', bg: '#ECFDF5', icon: <CheckCircle size={14} /> },
          { l: 'Packs disponibles', v: Object.keys(PACKS).length, color: '#4F46E5', bg: '#EEF2FF', icon: <Package size={14} /> },
      ...
      {/* Tab switcher */}
      <div style={{ display: 'flex', gap: 4, marginBottom: 20, background: 'var(--card-bg)', padding: 4, borderRadius: 12, border: '1px solid var(--card-border)', width: 'fit-content' }}>
        {[
          { key: 'packs', label: 'Packs groupés', icon: <Package size={13} /> },
          { key: 'plugins', label: 'Plugins individuels', icon: <Puzzle size={13} /> },
        ].map(tab => (
      ...
      {activeTab === 'packs' && (
        <div>
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', marginBottom: 14, textTransform: 'uppercase', letterSpacing: '0.06em', display: 'flex', alignItems: 'center', gap: 6 }}>
            <Sparkles size={12} style={{ color: '#D97706' }} />Bundles recommandés
          </div>
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontSize: 15, fontWeight: 800, color: '#fff', marginBottom: 4 }}>
                🎁 Économisez jusqu'à 25% avec nos packs
              </div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,.75)' }}>
                Plugins groupés, activés instantanément — pas de configuration supplémentaire.
              </div>
            </div>
            <div style={{ fontSize: 11, padding: '5px 14px', borderRadius: 20, background: 'rgba(255,255,255,.2)', color: '#fff', fontWeight: 700, flexShrink: 0 }}>
              Offre lancement
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 16 }}>
            {Object.values(PACKS).map(pack => (
              <PackCard
                key={pack.id}
                pack={pack}
                onInstall={id => installPackMutation.mutate(id)}
                isInstalling={installPackMutation.isPending && installPackMutation.variables === pack.id}
                isInstalled={liveInstalledPacks.has(pack.id)}
              />
            ))}
          </div>
          ...
          <div style={{ marginTop: 28 }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', marginBottom: 14, textTransform: 'uppercase', letterSpacing: '0.06em' }}>
              Détail des plugins inclus par pack
            </div>
            <div style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 14, overflow: 'hidden' }}>
              {Object.values(PACKS).map((pack, pi) => (
                <div key={pack.id} style={{ padding: '14px 18px', borderBottom: pi < Object.values(PACKS).length - 1 ? '1px solid var(--card-border)' : 'none' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                    <span style={{ fontSize: 18 }}>{pack.icon}</span>
                    <div>
                      <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>{pack.name}</span>
                      <span style={{ fontSize: 11, color: 'var(--text-muted)', marginLeft: 8 }}>{pack.price}</span>
                    </div>
                    {liveInstalledPacks.has(pack.id) && (
                      <span style={{ marginLeft: 'auto', fontSize: 11, padding: '2px 9px', borderRadius: 20, background: '#ECFDF5', color: '#059669', fontWeight: 700 }}>
                        ✓ Installé
                      </span>
                    )}
                  </div>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                    {pack.plugins.map(pluginId => (
                      <span key={pluginId} style={{
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 20, flexWrap: 'wrap' }}>
            {CATEGORIES.map(c => (
              <button key={c.value} onClick={() => setCategory(c.value)}
                style={{
                  display: 'flex', alignItems: 'center', gap: 5, padding: '6px 14px', borderRadius: 10, border: 'none', fontSize: 12, fontWeight: 500, cursor: 'pointer', transition: 'all .15s',
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: 12 }}>
                {featured.map(p => (
                  <div key={p.id} onClick={() => setModal(p)}
                    style={{ background: 'var(--card-bg)', border: '2px solid var(--color-primary)', borderRadius: 16, padding: '16px 18px', cursor: 'pointer', position: 'relative', overflow: 'hidden' }}>
                    ...
                    <p style={{ fontSize: 12, color: 'var(--text-muted)', margin: 0, lineHeight: 1.5 }}>{p.description}</p>
                    {installed.has(p.id) && (
          ...
          {filtered.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 12 }}>
              {filtered.map(p => {
                ...
                return (
                  <div key={p.id}
                    style={{ background: 'var(--card-bg)', border: '1px solid var(--card-border)', borderRadius: 16, padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 10, cursor: 'pointer' }}
                    onClick={() => setModal(p)}>
