...
export default function ProspectsPage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      {toastMsg && (
        <div className="fixed top-6 right-6 z-50 bg-teal-600 text-white px-4 py-3 rounded-xl shadow-lg text-sm flex items-center gap-2">
          <CheckCircle className="w-4 h-4"/>{toastMsg}
        </div>
      )}
      {showCreate && (
        <CreateModal onClose={()=>setShowCreate(false)} onSave={()=>{fetchProspects();showToast('Prospect créé ✓');}}/>
      )}
      {showImport && (
        <ImportCSVModal onClose={()=>setShowImport(false)} onSave={()=>{fetchProspects();showToast('Import réussi ✓');}}/>
      )}
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        <table className="w-full">
          ...
          <tbody className="divide-y divide-gray-100">
            ...
            {prospects.map(p=>(
              <tr key={p.id} className="hover:bg-gray-50 cursor-pointer" onClick={()=>navigate(`/prospects/${p.id}`)}>
