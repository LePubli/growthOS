...
// Appliquer les tokens CSS sur <html> immédiatement
function applyTokens(tokens: ThemeTokens, isDark: boolean) {
  if (typeof document === 'undefined') return;
  const root = document.documentElement;
  // Appliquer chaque token CSS
  Object.entries(tokens).forEach(([k, v]) => {
    root.style.setProperty(k, v);
  });
  // Data-attribute pour les sélecteurs CSS
...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  // ── Init : appliquer le thème sauvegardé DÈS le premier render ──────
  useEffect(() => {
  ...
  const setTheme = useCallback(async (id: string) => {
    ...
    if (!found) return;
    setIsLoading(true);
    // Appliquer IMMÉDIATEMENT (avant l'API call)
    applyTokens(found.tokens, found.isDark);
    setCurrent(found);
    localStorage.setItem(STORAGE_KEY, id);
    // Sync API (non bloquant)
    try {
    ...
    } catch { /* silencieux — l'application locale est déjà faite */ }
