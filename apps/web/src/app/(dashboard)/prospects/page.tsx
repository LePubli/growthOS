...
import {
  Search, Plus, Star, Mail, Phone, Globe, Filter,
...
Thomas,Martin,t.martin@techvision.io,,TechVision,CTO,https://techvision.io,new`;
  ...
  const parseCSV = (text:string) => {
    ...
    }).filter(r=>r.firstName||r.email||r.company);
  };
          ...
          <>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={importData} disabled={!file||loading} className="flex-1 py-2.5 bg-teal-600 text-white rounded-xl text-sm font-medium disabled:opacity-50 flex items-center justify-center gap-2">
                {loading?<Loader2 className="w-4 h-4 animate-spin"/>:<Upload className="w-4 h-4"/>}Importer {count>0?`(${count})`:''}
              </button>
            </div>
          </>
        )}
...
export default function ProspectsPage() {
  ...
  const byStatus = STATUS_OPTIONS.reduce((acc,s) => { if(s.value!=='all') acc[s.value]=prospects.filter(p=>p.status===s.value).length; return acc; },{} as Record<string,number>);
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="relative mb-4 max-w-sm">
        ...
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Rechercher par nom, email, entreprise..." className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-teal-500"/>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        <table className="w-full">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>{['','Nom','Entreprise','Email / Tél','Statut','Score','Actions'].map(h=><th key={h} className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">{h}</th>)}</tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {loading && prospects.length===0 && <tr><td colSpan={7} className="text-center py-16"><Loader2 className="w-8 h-8 animate-spin text-teal-600 mx-auto"/></td></tr>}
            ...
            {prospects.map(p=>(
              <tr key={p.id} className="hover:bg-gray-50 cursor-pointer" onClick={()=>router.push(`/prospects/${p.id}`)}>
                ...
                <td className="px-4 py-3">
                  {p.score>0 && (
                    <div className="flex items-center gap-2">
                      <div className="w-16 bg-gray-100 rounded-full h-1.5"><div className={`h-1.5 rounded-full ${p.score>=80?'bg-green-500':p.score>=50?'bg-amber-500':'bg-gray-400'}`} style={{width:`${p.score}%`}}/></div>
